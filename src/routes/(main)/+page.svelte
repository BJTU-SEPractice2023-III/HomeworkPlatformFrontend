...
<div class="flex w-full h-full">
  <div class="bg-white w-[200px] p-4 flex flex-col">
    <Button on:click={() => {}}>创建课程</Button>
    <div>教的课程</div>
    {#each teachingCourses as teachingCourse}
      <a
        href="/course?courseId={teachingCourse.ID}"
        class="text-black underline">{teachingCourse.name}</a
      >
    {/each}
    ...
    {#each learningcourses as learningcourse}
      <a
        href="/course?courseId={learningcourse.ID}"
        class="text-black underline">{learningcourse.name}</a
      >
    {/each}
